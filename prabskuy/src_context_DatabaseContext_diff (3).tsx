...
import {
  ...
  loadDummyData as dexieLoadDummy, resetAllData as dexieResetAll,
  Account, Transaction, Budget, Goal, GoalDeposit, Recurring, HabitLog, ChallengeState, AppSettings, Category,
  TxTemplate, InvestmentAsset, InvestmentTransaction, CustomCategory, BudgetWarning,
} from '../db/dexie';
import { pushUndo } from '../utils/features';
...
interface DatabaseContextType {
  ...
  deleteInvestment: (uid: string) => Promise<void>;
  addInvestmentTransaction: (tx: Omit<InvestmentTransaction, 'id' | 'uid' | 'createdAt'>) => Promise<void>;
  addCustomCategory: (cat: Omit<CustomCategory, 'id' | 'uid' | 'createdAt'>) => Promise<void>;
  deleteCustomCategory: (uid: string) => Promise<void>;
  addBudgetWarning: (warning: Omit<BudgetWarning, 'id' | 'uid' | 'createdAt'>) => Promise<void>;
  updateBudgetWarning: (uid: string, updates: Partial<BudgetWarning>) => Promise<void>;
  deleteBudgetWarning: (uid: string) => Promise<void>;
  resetAll: () => Promise<void>;
  loadDummy: () => Promise<void>;
}
...
export function DatabaseProvider({ children }: { children: React.ReactNode }) {
  ...
  const budgetWarnings = useLiveQuery(() => db.budgetWarnings.toArray(), [], []) || [];
  ...
  // Custom categories
  const addCustomCategory = useCallback(async (cat: Omit<CustomCategory, 'id' | 'uid' | 'createdAt'>) => {
    await db.customCategories.add({ ...cat, uid: `cat-${uid()}`, createdAt: new Date().toISOString() });
  }, []);

  const deleteCustomCategory = useCallback(async (catUid: string) => {
    await db.customCategories.where('uid').equals(catUid).delete();
  }, []);

  // Budget warnings
  const addBudgetWarning = useCallback(async (warning: Omit<BudgetWarning, 'id' | 'uid' | 'createdAt'>) => {
    await db.budgetWarnings.add({ ...warning, uid: `warn-${uid()}`, createdAt: new Date().toISOString() });
  }, []);

  const updateBudgetWarning = useCallback(async (warnUid: string, updates: Partial<BudgetWarning>) => {
    await db.budgetWarnings.where('uid').equals(warnUid).modify(updates);
  }, []);

  const deleteBudgetWarning = useCallback(async (warnUid: string) => {
    await db.budgetWarnings.where('uid').equals(warnUid).delete();
  }, []);
  ...
  const value: DatabaseContextType = {
    accounts, categories, transactions, budgets, goalDeposits, goals, recurrings, habitLog,
    challengeState, settings, templates, investments, investmentTransactions,
    customCategories, budgetWarnings, isLoading: !isReady,
    addTransaction, updateTransaction, deleteTransaction, deleteTransactions,
    addAccount, updateAccount, deleteAccount,
    addBudget, deleteBudget, addGoal, deleteGoal, addGoalDeposit,
    addRecurring, deleteRecurring, addHabitLog,
    ...
    addBudgetWarning, updateBudgetWarning, deleteBudgetWarning,
    resetAll, loadDummy,
  };

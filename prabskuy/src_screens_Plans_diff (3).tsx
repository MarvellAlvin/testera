...
export function Plans() {
  ...
  const handleAddBudget = () => {
    ...
    addBudget({ categoryId: budgetCategory, amount: formattedToNumber(budgetAmount), startDay: parseInt(budgetStartDay) || 1, alertPercent: 80 });
  ...
  const handleAddGoal = () => {
    ...
    addGoal({ name: goalName, targetAmount: formattedToNumber(goalTarget), currentAmount: 0, deadlineMonths: parseInt(goalMonths) || 12, icon: goalIcon });
  ...
  const handleAddRecurring = () => {
    ...
    addRecurring({ type: recType, note: recNote, amount: formattedToNumber(recAmount), accountId: recAccount, categoryId: recCategory, frequency: recFreq, nextDue: nextDue.toISOString().split('T')[0], autoPost: recAutoPost });
    setShowAddRecurring(false); setRecNote(''); setRecAmount('');
    hapticFeedback('light');
  };

  const handleDeposit = async (goalUid: string) => {
    const amount = formattedToNumber(depositAmount);
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C1C1E] rounded-2xl p-6 max-w-sm w-full border border-[#2C2C2E]">
            ...
            <div className="space-y-3">
              ...
              <FormattedNumberInput value={budgetAmount} onChange={setBudgetAmount} placeholder="Jumlah budget" prefix="Rp" />
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C1C1E] rounded-2xl p-6 max-w-sm w-full border border-[#2C2C2E]">
            ...
            <div className="space-y-3">
              ...
              <FormattedNumberInput value={goalTarget} onChange={setGoalTarget} placeholder="Target jumlah" prefix="Rp" />
      ...
      {showDeposit && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C1C1E] rounded-2xl p-6 max-w-sm w-full border border-[#2C2C2E]">
            ...
            <FormattedNumberInput value={depositAmount} onChange={setDepositAmount} placeholder="Jumlah setor" prefix="Rp" />
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C1C1E] rounded-2xl p-6 max-w-sm w-full border border-[#2C2C2E] max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <FormattedNumberInput value={recAmount} onChange={setRecAmount} placeholder="Jumlah" prefix="Rp" />
...
export function Plans() {
  ...
  const handleAddRecurring = () => {
    ...
    else if (recFreq === 'monthly') nextDue.setMonth(nextDue.getMonth() + 1);
    else if (recFreq === 'yearly') nextDue.setFullYear(nextDue.getFullYear() + 1);
    addRecurring({ type: recType, note: recNote, amount: formattedToNumber(recAmount), accountId: recAccount, categoryId: recCategory, frequency: recFreq, nextDue: nextDue.toISOString().split('T')[0], autoPost: recAutoPost });
    setShowAddRecurring(false); setRecNote(''); setRecAmount('');
    hapticFeedback('light');
  };
  ...
  const tabs: { key: PlansTab; label: string; icon: React.ReactNode }[] = [
    { key: 'budgets', label: 'Budget', icon: <Target size={16} /> },
    { key: 'goals', label: 'Goals', icon: <PiggyBank size={16} /> },
    { key: 'recurring', label: 'Rutinan', icon: <Repeat size={16} /> },
  ];
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {showAddRecurring && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C1C1E] rounded-2xl p-6 max-w-sm w-full border border-[#2C2C2E] max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold text-[#F5F5F5]">Tambah Rutinan</h3><button onClick={() => setShowAddRecurring(false)}><X size={20} className="text-[#9CA3AF]" /></button></div>
            <div className="space-y-3">
              <div className="flex gap-2">
                <button onClick={() => setRecType('expense')} className={`flex-1 py-2 rounded-xl text-sm ${recType === 'expense' ? 'bg-[#FB923C]/20 text-[#FB923C] border border-[#FB923C]/30' : 'bg-[#262626] text-[#9CA3AF]'}`}>Pengeluaran</button>
                <button onClick={() => setRecType('income')} className={`flex-1 py-2 rounded-xl text-sm ${recType === 'income' ? 'bg-[#22C55E]/20 text-[#22C55E] border border-[#22C55E]/30' : 'bg-[#262626] text-[#9CA3AF]'}`}>Pemasukan</button>

...
// Investment assets - separate from regular accounts
export interface InvestmentAsset {
  id?: number;
  uid: string;
  name: string;
  type: 'saham' | 'saham_id' | 'etf' | 'reksadana' | 'obligasi' | 'crypto' | 'emas' | 'properti' | 'deposito' | 'lainnya';
  icon: string;
  purchasePrice: number; // harga beli per unit (per lembar untuk saham ID, per gram untuk emas, per koin untuk crypto)
  quantity: number; // jumlah unit (lot untuk saham ID, gram untuk emas, koin untuk crypto)
  currentValue: number; // nilai awal saat dicatat (total)
  purchaseDate: string;
  market?: 'IDX' | 'GLOBAL'; // IDX = saham Indonesia (1 lot = 100 lembar)
  notes?: string;
  // Reksadana specific
  returnPercent?: number; // persentase return
  // Deposito specific
  interestPercent?: number; // persentase bunga per tahun
  depositDuration?: number; // lama deposito dalam hari
  maturityDate?: string; // tanggal jatuh tempo
  autoRenew?: boolean; // auto perpanjang saat jatuh tempo
  createdAt: string;
}
...
// Custom categories - user can create their own
...
export interface BudgetWarning {
  ...
  uid: string;
  budgetUid: string;
  thresholdAmount: number; // warn when remaining is below this amount
  enabled: boolean;
  createdAt: string;
}
...
class FinanceDB extends Dexie {
  ...
  budgetWarnings!: EntityTable<BudgetWarning, 'id'>;
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      settings: '++id, &uid',
      investments: '++id, &uid, type, purchaseDate',
      investmentTransactions: '++id, &uid, assetUid, type, date',
      customCategories: '++id, &uid, type',
      budgetWarnings: '++id, &uid, budgetUid',
    });
  }
}
...
export interface InvestmentAsset {
  ...
  market?: 'IDX' | 'GLOBAL';
  notes?: string;
  research?: string; // Investment research & analysis
  targetPrice?: number;
  stopLoss?: number;
...
// Investment Journal - Monthly reflections
...
// Investment Streaks - Monthly tracking
...
class FinanceDB extends Dexie {
  ...
  customCategories!: EntityTable<CustomCategory, 'id'>;
  budgetWarnings!: EntityTable<BudgetWarning, 'id'>;
  investmentJournals!: EntityTable<InvestmentJournal, 'id'>;
  achievements!: EntityTable<Achievement, 'id'>;
  investmentStreaks!: EntityTable<InvestmentStreak, 'id'>;
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      customCategories: '++id, &uid, type',
      budgetWarnings: '++id, &uid, budgetUid',
      investmentJournals: '++id, &uid, month',
      achievements: '++id, &uid, key, category',
      investmentStreaks: '++id, &uid, month',
      userProfiles: '++id, &uid, isDefault',
    });
  }
}

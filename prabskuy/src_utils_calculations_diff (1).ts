...
// Business logic calculations - works with Dexie types (uid-based)
import type { Transaction, Account, Category, Budget, Goal, GoalDeposit, HabitLog } from '../db/dexie';
...
export function calculateAccountBalances(accounts: Account[], transactions: Transaction[]): AccountBalance[] {
  return accounts.map(account => {
    const accUid = account.uid;
    const income = transactions.filter(t => t.type === 'income' && t.accountId === accUid).reduce((s, t) => s + t.amount, 0);
    const expense = transactions.filter(t => t.type === 'expense' && t.accountId === accUid).reduce((s, t) => s + t.amount, 0);
    const transferIn = transactions.filter(t => t.type === 'transfer' && t.toAccountId === accUid).reduce((s, t) => s + t.amount, 0);
    const transferOut = transactions.filter(t => t.type === 'transfer' && t.accountId === accUid).reduce((s, t) => s + t.amount, 0);
    return {
      uid: accUid,
      name: account.name,
      icon: account.icon,
      openingBalance: account.openingBalance,
      balance: account.openingBalance + income + transferIn - expense - transferOut,
      isSavings: account.isSavings,
    };
  });
}
...
// Q2: Period summary
export interface PeriodSummary { income: number; expense: number; net: number; }
...
// Q3: Top categories
export interface CategoryTotal { uid: string; name: string; icon: string; total: number; percentage: number; }

export function calculateTopCategories(transactions: Transaction[], categories: Category[], from: string, to: string, limit: number = 6): CategoryTotal[] {
  const filtered = transactions.filter(t => t.date >= from && t.date <= to && t.type === 'expense' && t.categoryId);
  const catMap = new Map(categories.map(c => [c.uid, c]));
  const totals = new Map<string, number>();
  filtered.forEach(t => { if (t.categoryId) totals.set(t.categoryId, (totals.get(t.categoryId) || 0) + t.amount); });
  const totalExpense = filtered.reduce((s, t) => s + t.amount, 0);
  return Array.from(totals.entries())
    .map(([catUid, total]) => {
      const cat = catMap.get(catUid);
      return { uid: catUid, name: cat?.name || 'Unknown', icon: cat?.icon || '📦', total, percentage: totalExpense > 0 ? (total / totalExpense) * 100 : 0 };
    })
    .sort((a, b) => b.total - a.total)
    .slice(0, limit);
}
...
// Q4: Cashflow buckets
export interface BarBucket { label: string; date: string; income: number; expense: number; }
...
// Budget
export interface BudgetStatus { budget: Budget; spent: number; percentage: number; remaining: number; isOver: boolean; overAmount: number; }
...
export function calculateBudgetStatus(budget: Budget, transactions: Transaction[], category: Category | undefined): BudgetStatus {
  ...
  const from = cycleStart.toISOString().split('T')[0];
  const to = cycleEnd.toISOString().split('T')[0];
  const spent = transactions.filter(t => t.type === 'expense' && t.categoryId === budget.categoryId && t.date >= from && t.date < to).reduce((s, t) => s + t.amount, 0);
  const percentage = budget.amount > 0 ? (spent / budget.amount) * 100 : 0;
  return { budget, spent, percentage, remaining: budget.amount - spent, isOver: spent > budget.amount, overAmount: spent > budget.amount ? spent - budget.amount : 0 };
}

// Goal
export interface GoalStatus { goal: Goal; percentage: number; estimatedMonthsLeft: number; monthlyNeeded: number; isLate: boolean; lateDays: number; }

export function calculateGoalStatus(goal: Goal, deposits: GoalDeposit[]): GoalStatus {
  const totalDeposits = deposits.filter(d => d.goalId === goal.uid).reduce((s, d) => s + d.amount, 0);
  const currentAmount = goal.currentAmount + totalDeposits;
  const percentage = goal.targetAmount > 0 ? (currentAmount / goal.targetAmount) * 100 : 0;
  const remaining = goal.targetAmount - currentAmount;
  const createdAt = new Date(goal.createdAt);
...
// Habit
export interface HabitStatus { consistencyPercent: number; streak: number; stage: string; stageEmoji: string; todayLogged: boolean; }

export function calculateHabitStatus(habitLog: HabitLog[], transactions: Transaction[]): HabitStatus {
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const todayStr = today.toISOString().split('T')[0];
  const todayLogged = habitLog.some(h => h.date === todayStr) || transactions.some(t => t.date === todayStr && t.type === 'expense');
  const last30Days: string[] = [];
  for (let i = 0; i < 30; i++) { const d = new Date(today); d.setDate(d.getDate() - i); last30Days.push(d.toISOString().split('T')[0]); }
  const loggedDates = new Set(habitLog.map(h => h.date));
  const loggedDays = last30Days.filter(d => loggedDates.has(d)).length;
  const consistencyPercent = (loggedDays / 30) * 100;
  let streak = 0;
  for (let i = 0; i < 365; i++) {
    ...
    const dateStr = d.toISOString().split('T')[0];
    if (loggedDates.has(dateStr) || transactions.some(t => t.date === dateStr && t.type === 'expense')) streak++;
    else if (i > 0) break;
  }
  let stage: string, stageEmoji: string;
  if (consistencyPercent < 20) { stage = 'Benih'; stageEmoji = '🌰'; }
  else if (consistencyPercent < 40) { stage = 'Tunas'; stageEmoji = '🌱'; }
  else if (consistencyPercent < 60) { stage = 'Pucuk'; stageEmoji = '🌿'; }
  else if (consistencyPercent < 80) { stage = 'Pohon Muda'; stageEmoji = '🪴'; }
